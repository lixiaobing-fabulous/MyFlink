...
import com.lxb.flink.configuration.description.TextElement;
...
import static com.lxb.flink.configuration.description.TextElement.code;
import static com.lxb.flink.configuration.description.TextElement.text;
...
public class PipelineOptions {
	...
	public static final ConfigOption<ExecutionConfig.ClosureCleanerLevel> CLOSURE_CLEANER_LEVEL =
		...
		.withDescription(Description.builder()
			...
			.list(
				text("%s - disables the closure cleaner completely", code(ExecutionConfig.ClosureCleanerLevel.NONE.toString())),
				text(
					"%s - cleans only the top-level class without recursing into fields",
					code(ExecutionConfig.ClosureCleanerLevel.TOP_LEVEL.toString())),
				text("%s - cleans all the fields recursively", code(ExecutionConfig.ClosureCleanerLevel.RECURSIVE.toString()))
			)
			.build());
	...
	public static final ConfigOption<List<String>> KRYO_DEFAULT_SERIALIZERS =
		key("pipeline.default-kryo-serializers")
			...
			.withDescription(Description.builder()
				...
				.text("Example:")
				.linebreak()
				.add(code("class:org.example.ExampleClass,serializer:org.example.ExampleSerializer1;" +
	...
	public static final ConfigOption<List<String>> CACHED_FILES =
		key("pipeline.cached-files")
			...
			.withDescription(Description.builder()
				...
				.text("Example:")
				.linebreak()
				.add(code("name:file1,path:`file:///tmp/file1`;name:file2,path:`hdfs:///tmp/file2`"))
				.build());
}
